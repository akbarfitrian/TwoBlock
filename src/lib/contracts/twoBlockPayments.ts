...
import type { Billing } from "@/hooks/useVerification";

/// Mirrors contracts/TwoBlockPayments.sol. Keep this in sync if the ABI changes.
...
// Solidity `Tier` enum order: Verified = 0, VerifiedPro = 1, VerifiedMax = 2.
// "free" is never purchased on-chain, so it has no on-chain index.
...
// Solidity `Billing` enum order: Monthly = 0, Yearly = 1.
